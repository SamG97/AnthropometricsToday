...
conn = psycopg2.connect(
    "dbname='group project' user='postgres' host='localhost' password='postgres'")
cur = conn.cursor()
...
WHERE TRUE """
            ...
            if (len(oldrows) > 20 or len(
                    oldrows) == oldtablesize or delta > 200000):
                break
            else:
                delta = delta * 2
...
WHERE TRUE """
        ...
        if (len(newrows) > 20 or len(
                newrows) == newtablesize or delta > 200000):
            break
        else:
            delta = delta * 2
